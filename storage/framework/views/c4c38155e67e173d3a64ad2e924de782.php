<?php $__env->startSection('title', 'Historial de Órdenes de Compra'); ?>
...
<div class="max-w-7xl mx-auto p-6 mt-20 bg-white/95 rounded-2xl shadow-2xl border border-slate-200 ring-1 ring-slate-100 oc-scope">
    ...
    <div class="overflow-x-auto thin-scrollbar">
        <table id="tablaOC" class="w-full border-collapse bg-white rounded-lg overflow-hidden shadow-sm">
            ...
            <?php $ordersGrandTotal = 0; ?>
            <tbody class="text-gray-700">
                <?php $__currentLoopData = $ordenes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $oc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                <tr class="border-b odd:bg-white even:bg-slate-50 hover:bg-indigo-50/40 transition">
                    <td class="p-3 whitespace-nowrap text-sm" style="width:100px;">#<?php echo e($oc->requisicion->id ?? '-'); ?></td>
                    <td class="p-3"><?php echo e($oc->order_oc ?? ('OC-' . $oc->id)); ?></td>
                    <td class="p-3"><?php echo e(optional($oc->created_at)->format('d/m/Y H:i')); ?></td>
                    <td class="p-3"><?php echo e($proveedor->prov_name ?? '—'); ?></td>
                    <td class="p-3"><?php echo e(optional($proveedor)->methods_oc ?? $oc->methods_oc ?? '—'); ?></td>
                    <td class="p-3"><?php echo e(optional($proveedor)->plazo_oc ?? $oc->plazo_oc ?? '—'); ?></td>
                    <td class="p-3 text-right font-semibold"><?php echo e(number_format($ocTotal, 2)); ?></td>
                    <?php
                        // clase por defecto
                        $estatusLower = strtolower(trim((string)$estatusDisplay));
                    ...
                    ?>
                    <td class="p-3">
                        <span class="inline-flex flex-col items-center justify-center px-3 py-1 rounded-full text-xs font-semibold <?php echo e($badgeClass); ?>"><?php echo implode('<br>', array_map('e', preg_split('/\s+/', $estatusDisplay))); ?></span>
                    </td>
                    <td class="p-3 text-center">
                        <div class="flex justify-center gap-2 items-center">
                            <button type="button" data-oc-id="<?php echo e($oc->id); ?>" class="btn-open-ver bg-blue-600 hover:bg-blue-700 text-white rounded p-2 w-9 h-9 flex items-center justify-center shadow" title="Ver OC" aria-label="Ver OC">
                                <i class="fas fa-eye"></i>
                            </button>
                            <?php if(!($isTerminada ?? false)): ?>
                            <button type="button" data-oc-id="<?php echo e($oc->id); ?>" class="btn-open-recibir bg-yellow-600 hover:bg-yellow-700 text-white rounded p-2 w-9 h-9 flex items-center justify-center shadow" title="Recibir productos" aria-label="Recibir productos">
                                <i class="fas fa-box"></i>
                            </button>
                            <?php endif; ?>
                            <?php if(!($isTerminada ?? false)): ?>
                            <a href="<?php echo e(route('ordenes_compra.edit', $oc->id)); ?>" title="Editar OC" class="bg-orange-500 hover:bg-orange-600 text-white rounded p-2 w-9 h-9 flex items-center justify-center shadow" aria-label="Editar OC">
                                <i class="fas fa-edit"></i>
                            </a>
                            <?php endif; ?>
                            <?php if(!($isTerminada ?? false)): ?>
                            <button type="button" data-oc-id="<?php echo e($oc->id); ?>" data-terminar-url="<?php echo e(route('ordenes_compra.terminar', ['id' => $oc->id], false)); ?>" class="btn-terminar-oc bg-red-600 hover:bg-red-700 text-white rounded p-2 w-9 h-9 flex items-center justify-center shadow" title="Terminar OC" aria-label="Terminar OC">
                                <i class="fas fa-flag-checkered"></i>
                            </button>
                            <?php endif; ?>
                            
                            <button type="button" data-oc-id="<?php echo e($oc->id); ?>" data-precios-url="/ordenes_compra/actualizar-precios-factura" class="btn-open-precios-factura bg-cyan-600 hover:bg-cyan-700 text-white rounded p-2 w-9 h-9 flex items-center justify-center shadow" title="Editar precios de factura" aria-label="Editar precios de factura">
                                <i class="fas fa-file-invoice-dollar"></i>
                            </button>
                            <?php if($showCreate): ?>
                                <a href="<?php echo e(route('ordenes_compra.create', ['requisicion_id' => $requisicionId, 'producto_id' => $pendingProductId, 'cantidad' => $pendingQty])); ?>" title="Crear nueva OC" class="bg-indigo-600 hover:bg-indigo-700 text-white rounded p-2 w-9 h-9 flex items-center justify-center shadow" aria-label="Crear nueva OC">
                                    <i class="fas fa-plus"></i>
                                </a>
                            <?php endif; ?>
                            <button type="button" class="btn-download-oc-pdf bg-green-600 hover:bg-green-700 text-white rounded p-2 w-9 h-9 flex items-center justify-center shadow" title="Descargar PDF" aria-label="Descargar PDF" data-href="<?php echo e(route('ordenes_compra.download', $requisicionId, false)); ?>">
    ...
    <?php $__currentLoopData = $ordenes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $oc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php $requisicionId = $oc->requisicion->id ?? ($oc->requisicion_id ?? null); ?>

        <!-- Modal recibir productos -->
        <div id="modal-recibir-oc-<?php echo e($oc->id); ?>" class="fixed inset-0 z-[9999] hidden items-center justify-center p-4" data-oc-id="<?php echo e($oc->id); ?>" data-requisicion-id="<?php echo e($requisicionId); ?>">
            ...
            <div class="relative bg-white w-full max-w-3xl rounded-lg shadow-lg overflow-hidden flex flex-col rc-modal">
                <div class="flex justify-between items-center px-6 py-4 border-b">
                    <h3 class="text-lg font-semibold">Recibir productos de la OC <?php echo e($oc->order_oc ?? ('OC-'.$oc->id)); ?></h3>
                    <button type="button" class="text-gray-600 hover:text-gray-800 rc-close" data-oc-id="<?php echo e($oc->id); ?>">✕</button>
                </div>
                <div class="p-6 overflow-y-auto max-h-[70vh] thin-scrollbar">
                    ...
                    <div class="overflow-x-auto rc-table-wrapper thin-scrollbar">
                        <table class="w-full text-sm border rounded overflow-hidden bg-white rc-table">
                            ...
                            <tbody>
                                <?php $__currentLoopData = $recRows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                ...
                                ?>
                                <tr class="border-t rc-row" data-rec-id="<?php echo e($r->recepcion_id ?? ''); ?>" data-producto-id="<?php echo e($r->producto_id); ?>" data-total="<?php echo e((int)$r->cantidad_total); ?>" data-current="<?php echo e((int)$r->recibido); ?>">
                                    <td class="p-2 align-top col-prod" title="<?php echo e($r->name_produc); ?>"><span class="truncate block"><?php echo e($r->name_produc); ?></span></td>
                                    <td class="p-2 text-center align-top"><?php echo e((int)$r->cantidad_total); ?></td>
                                    <td class="p-2 text-center align-top"><?php echo e($r->unit_produc ?? '—'); ?></td>
                                    <td class="p-2 text-center align-top"><?php echo e(number_format($price, 2)); ?></td>
                                    <td class="p-2 text-center align-top"><?php echo e(number_format($lineTotal, 2)); ?></td>
                                    <td class="p-2 text-center align-top"><?php echo e((int)$r->recibido); ?></td>
                                    <td class="p-2 text-center align-top"><?php echo e($pend); ?> <?php if($pend === 0): ?> <span class="ml-2 px-2 py-1 text-xs bg-green-100 text-green-700 rounded">Recepción completada</span> <?php endif; ?></td>
                                    <td class="p-2 text-center align-top">
                                        <input type="number" min="0" max="<?php echo e($pend); ?>" value="<?php echo e($pend); ?>" class="w-24 border rounded p-1 text-center rcx-input" <?php echo e($pend === 0 ? 'disabled' : ''); ?>>
                                    </td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                            <tfoot>
                                <tr class="bg-gray-50 font-semibold border-t">
                                    ...
                                    <td class="p-2 text-center"><?php echo e(number_format($grandRecTotal, 2)); ?></td>
                                    <td colspan="3"></td>
                                </tr>
                            </tfoot>
                        </table>
                    ...
                    <div class="flex justify-end gap-3 mt-4">
                        <button type="button" class="px-4 py-2 border rounded rc-cancel" data-oc-id="<?php echo e($oc->id); ?>">Cancelar</button>
                        <button type="button" class="px-4 py-2 bg-blue-600 text-white rounded rc-save" data-oc-id="<?php echo e($oc->id); ?>" data-confirm-url="<?php echo e(route('recepciones.confirmar', [], false)); ?>">Guardar recepción</button>
                    </div>
                    <?php else: ?>
        ...
        <div id="modal-precios-factura-<?php echo e($oc->id); ?>" class="fixed inset-0 z-[9999] hidden items-center justify-center p-4" data-oc-id="<?php echo e($oc->id); ?>">
            <div class="absolute inset-0 bg-black/50" data-close="1"></div>
            <div class="relative bg-white w-full max-w-4xl rounded-lg shadow-lg overflow-hidden flex flex-col">
                <div class="flex justify-between items-center px-6 py-4 border-b">
                    <h3 class="text-lg font-semibold">Editar precios de factura - <?php echo e($oc->order_oc ?? ('OC-'.$oc->id)); ?></h3>
                    <button type="button" class="text-gray-600 hover:text-gray-800 pf-close" data-oc-id="<?php echo e($oc->id); ?>">✕</button>
                </div>
                <div class="p-6 thin-scrollbar overflow-y-auto max-h-[75vh]">
                    <?php $lines = $oc->ordencompraProductos; ?>
                    <?php if(($lines ?? collect())->count()): ?>
                        ...
                        <tbody>
                            <?php $__currentLoopData = $lines; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ln): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                    ...
                                    $trm   = $ln->trm_factura;
                                ?>
                                <tr class="border-t pf-row" data-ocp-id="<?php echo e($ln->id); ?>" data-proveedor-id="<?php echo e($ln->proveedor_id); ?>">
                                    <td class="p-2"><?php echo e(optional($ln->producto)->name_produc ?? ('#'.$ln->producto_id)); ?></td>
                                    <td class="p-2">
                                        <select class="pf-proveedor border rounded p-1 w-full text-sm" data-ocp-id="<?php echo e($ln->id); ?>">
                                            <?php $allProveedores = \App\Models\Proveedor::all(); ?>
                                            <?php $__currentLoopData = $allProveedores; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prov): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($prov->id); ?>" <?php echo e($ln->proveedor_id == $prov->id ? 'selected' : ''); ?>>
                                                    <?php echo e($prov->prov_name); ?>

                                                </option>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                        </select>
                                    </td>
                                    <td class="p-2 text-center"><?php echo e($cant); ?></td>
                                    <td class="p-2 text-center">
                                        <input type="number" step="0.01" min="0" class="pf-price border rounded p-1 w-28 text-right" value="<?php echo e(!is_null($pFac) ? number_format((float)$pFac, 2, '.', '') : ''); ?>" placeholder="0.00">
                                    </td>
                                    <td class="p-2 text-center">
                                        <input type="number" step="0.01" min="0" class="pf-trm border rounded p-1 w-28 text-right" value="<?php echo e(!is_null($trm) ? number_format((float)$trm, 2, '.', '') : ''); ?>" placeholder="1.00">
                                    </td>
                                </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </tbody>
                    ...
                    <div class="flex justify-end gap-3 mt-4">
                        <button type="button" class="px-4 py-2 border rounded pf-close" data-oc-id="<?php echo e($oc->id); ?>">Cancelar</button>
                        <button type="button" class="px-4 py-2 bg-cyan-600 text-white rounded pf-save" data-oc-id="<?php echo e($oc->id); ?>">Guardar</button>
                    </div>
                    <?php else: ?>
                        <div class="text-gray-600">Esta orden no tiene líneas.</div>
                    <?php endif; ?>
    ...
    <?php $__currentLoopData = $ordenes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $oc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div id="modal-<?php echo e($oc->id); ?>" class="fixed inset-0 z-[9999] hidden items-center justify-center p-4" data-modal="ver">
        <div class="absolute inset-0 bg-black/50" data-close="1"></div>
        <div class="relative w-full max-w-4xl">
            <div class="bg-white rounded-2xl shadow-2xl max-h-[85vh] flex flex-col relative">
                <button onclick="toggleModal('modal-<?php echo e($oc->id); ?>')"
                ...
                <div class="overflow-y-auto p-8 thin-scrollbar" style="max-height:calc(85vh - 92px);">
                    <h2 class="text-2xl font-bold text-gray-800 mb-6 border-b pb-3">Orden <?php echo e($oc->order_oc ?? ('OC-' . $oc->id)); ?></h2>
                    ...
                    <section class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm bg-gray-50 rounded-lg p-4">
                            <div><span class="font-medium">Número de Orden:</span> <?php echo e($oc->order_oc ?? ('OC-' . $oc->id)); ?></div>
                            <div><span class="font-medium">Fecha de creación:</span> <?php echo e(optional($oc->created_at)->format('d/m/Y H:i')); ?></div>
                            <div><span class="font-medium">Requisición:</span> #<?php echo e($oc->requisicion->id ?? '-'); ?></div>
                            <div><span class="font-medium">Proveedor:</span> <?php echo e(optional(optional($oc->ordencompraProductos->first())->proveedor)->prov_name ?? '—'); ?></div>
                            <div><span class="font-medium">Método de pago:</span> <?php echo e(optional(optional($oc->ordencompraProductos->first())->proveedor)->methods_oc ?? $oc->methods_oc ?? '—'); ?></div>
                            <div><span class="font-medium">Plazo de pago:</span> <?php echo e(optional(optional($oc->ordencompraProductos->first())->proveedor)->plazo_oc ?? $oc->plazo_oc ?? '—'); ?></div>
                            <div><span class="font-medium">Fecha Estimada Recepción:</span> <?php echo e($oc->fecha_estimada_recepcion ? \Carbon\Carbon::parse($oc->fecha_estimada_recepcion)->format('d/m/Y') : '—'); ?></div>
                            <?php if(!empty($oc->observaciones)): ?>
                            <div class="md:col-span-2"><span class="font-medium">Observaciones:</span> <?php echo e($oc->observaciones); ?></div>
                            <?php endif; ?>
                        </div>
                    </section>
                            ...
                            <div>
                                <?php $grandTotal = 0; ?>
                                 <table class="w-full text-sm bg-white">
                                     ...
                                     <tbody>
                                         <?php $__currentLoopData = $oc->ordencompraProductos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $linea): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                         <?php if($linea->producto): ?>
                                         <?php
                                         ...
                                         ?>
                                         <tr class="border-b">
                                             <td class="p-3 font-medium text-gray-800 align-top"><?php echo e($linea->producto->name_produc); ?></td>
                                             <td class="p-3 text-center align-top"><?php echo e((int)$linea->total); ?></td>
                                             <td class="p-3 text-center align-top"><?php echo e($unitName); ?></td>
                                             <td class="p-3 text-center align-top"><?php echo e(number_format((float)$unitPrice, 2)); ?></td>
                                             <td class="p-3 text-center align-top"><?php echo e(number_format($lineTotal, 2)); ?></td>
                                             <td class="p-3 align-top">
                                                 ...
                                                 <ul class="list-disc list-inside text-sm text-gray-700 space-y-0.5">
                                                     <?php $__empty_1 = true; $__currentLoopData = $dist; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                                     <li><?php echo e($d->name_centro); ?> (<?php echo e($d->amount); ?>)</li>
                                    ...
                                    <tfoot>
                                        <tr class="bg-gray-50 font-semibold border-t">
                                            ...
                                            <td class="p-3 text-center"><?php echo e(number_format($grandTotal, 2)); ?></td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
                                 </table>
                ...
                <div class="sticky bottom-0 left-0 bg-white pt-4 pb-4 px-8 flex flex-wrap gap-3 justify-end border-t z-20">
                    <button type="button" class="bg-purple-600 text-white px-5 py-2 rounded-lg hover:bg-purple-700 transition flex items-center gap-1 btn-open-estatus-oc" data-oc-id="<?php echo e($oc->id); ?>">
                ...
                <?php
                    $recepcionesOC = DB::table('recepcion as r')
                        ...
                        ->select('r.id','r.created_at','r.cantidad','r.cantidad_recibido','r.reception_user','p.name_produc')
                         ...
                         ->get();
                ?>
                <div id="modal-estatus-oc-<?php echo e($oc->id); ?>" class="fixed inset-0 z-[10000] hidden items-center justify-center p-4">
                    <div class="absolute inset-0 bg-black/50" data-close="1"></div>
                    <div class="relative w-full max-w-3xl">
                        <div class="bg-white rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto p-6 relative thin-scrollbar">
                            <button onclick="toggleModal('modal-estatus-oc-<?php echo e($oc->id); ?>')" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 text-2xl">✕</button>
                            ...
                            <h3 class="text-xl font-semibold mb-4">Historial de estatus - OC <?php echo e($oc->order_oc ?? ('OC-'.$oc->id)); ?></h3>

                            <div class="mb-4">
                                <?php if(isset($estatusHist) && $estatusHist->count()): ?>
                                    <div class="mb-4">
                                        ...
                                        <?php $__currentLoopData = $estatusGrouped; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $groupKey => $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php
                                            ...
                                            <details class="mb-2">
                                                <summary class="p-4 bg-gray-50 border rounded flex justify-between items-center cursor-pointer">
                                                    <div class="font-medium"><?php echo e($displayName); ?></div>
                                                    <div class="flex items-center gap-2">
                                                        <div class="text-sm text-gray-600"><?php echo e($displayDate); ?></div>
                                                        ...
                                                        <table class="w-full text-sm bg-white">
                                                            ...
                                                            <tbody>
                                                                <?php $__currentLoopData = $group; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                    <tr>
                                                                        <td class="p-2"><?php echo e($row->producto_nombre ?? '—'); ?></td>
                                                                        <td class="p-2 text-center"><?php echo e($row->cantidad_recibido ?? $row->cantidad ?? 0); ?></td>
                                                                        <td class="p-2"><?php echo e($row->reception_user ?? '—'); ?></td>
                                                                    </tr>
                                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                            </tbody>
                                                        </table>

                                                        <?php if($firstRow->estatus_id != 3 && !empty($firstRow->comentario)): ?>
                                                            <div class="mt-3 text-sm text-gray-700">Comentario: <?php echo e($firstRow->comentario); ?></div>
                                                        <?php endif; ?>
                                                    <?php else: ?>
                                                        
                                                        <?php if(isset($firstRow->estatus_id) && (int)$firstRow->estatus_id !== 3): ?>
                                                            <div class="text-sm text-gray-700">Usuario: <?php echo e($firstRow->user_name ?? ($firstRow->user_id ?? '—')); ?></div>
                                                        <?php endif; ?>
                                                        <?php if(!empty($firstRow->comentario)): ?>
                                                            <div class="text-sm text-gray-700 mt-2">Comentario: <?php echo e($firstRow->comentario); ?></div>
                                                        <?php endif; ?>
                                                    <?php endif; ?>

                                                    <?php if(isset($firstRow->estatus_id) && (int)$firstRow->estatus_id === 3): ?>
                                                        <div class="mt-3 p-3 bg-green-50 border rounded text-sm text-gray-700">
                                                            <div>Fecha primer estatus: <?php echo e($firstStatusDate ? $firstStatusDate->format('d/m/Y H:i') : '—'); ?></div>
                                                            <div>Fecha último estatus: <?php echo e($lastStatusDate ? $lastStatusDate->format('d/m/Y H:i') : '—'); ?></div>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
                                            </details>
                                ...
                                <?php else: ?>
                                    <div class="p-4 bg-gray-50 border rounded flex justify-between items-center mb-3">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <div class="text-sm text-gray-600"><?php echo e(optional($oc->created_at)->format('d/m/Y H:i')); ?></div>
                                        </div>
                                    </div>
                                    <div class="p-4 border rounded bg-white mb-3">
                                        <div class="text-sm text-gray-700 font-medium">Usuario</div>
                                        <div class="text-sm text-gray-700 mt-1"><?php echo e(session('user.name') ?? session('user.email') ?? '—'); ?></div>
                                    </div>
                                <?php endif; ?>

                                <?php if($recepcionesOC->count() && empty($hasRecepcionInEstatus)): ?>
                                    <?php $__currentLoopData = $recepcionesOC; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rec): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <details class="mb-3">
                                            <summary class="p-4 bg-gray-50 border rounded flex justify-between items-center cursor-pointer">
                                                <div class="font-semibold">Recepción</div>
                                                <div class="flex items-center gap-2">
                                                    <div class="text-sm text-gray-600"><?php echo e(!empty($rec->created_at) ? \Carbon\Carbon::parse($rec->created_at)->format('d/m/Y H:i') : (!empty($rec->fecha) ? \Carbon\Carbon::parse($rec->fecha)->format('d/m/Y') : '—')); ?></div>
                                                    <svg class="details-summary-arrow w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z" clip-rule="evenodd"></path></svg>
                                                </div>
                                            </summary>
                                            <div class="p-4 border rounded mt-2 bg-white">
                                                <table class="w-full text-sm bg-white">
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td class="p-2"><?php echo e($rec->name_produc); ?></td>
                                                            <td class="p-2 text-center"><?php echo e($rec->cantidad_recibido ?? $rec->cantidad ?? 0); ?></td>
                                                            <td class="p-2"><?php echo e($rec->reception_user ?? '—'); ?></td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
        ...
        const init = function(){
            ...
            document.addEventListener('click', async function(e){
                ...
                if (btnSave) {
                    ...
                    const endpoint = btnSave.dataset.confirmUrl || `<?php echo e(route('recepciones.confirmar')); ?>`;
                    const modal = document.getElementById(`modal-recibir-oc-${ocId}`);
                    ...
                    try {
                        const receptionUser = <?php echo json_encode(session('user.name') ?? session('user.email') ?? session('user.id') ?? ''); ?>;
                ...
                if (btnTerm) {
                    ...
                    const endpoint = btnTerm.dataset.terminarUrl || `<?php echo e(url('/ordenes_compra/terminar')); ?>/${ocId}`;
                    if (window.Swal){
                    ...
                    try {
                        const resp = await fetch(endpoint, { method: 'POST', credentials: 'same-origin', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'Accept': 'application/json' } });
                        const data = await resp.json();
                ...
                if (btnFinReq) {
                    ...
                    const endpoint = btnFinReq.dataset.finalizarUrl || `<?php echo e(url('/requisiciones')); ?>/${reqId}/finalizar`;
                    if (window.Swal){
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Requisicion\resources\views/ordenes_compra/historial.blade.php ENDPATH**/ ?>