        ...
        <div class="max-w-7xl mx-auto main-card p-6 flex flex-col min-h-[80vh]">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('ordenes_compra.show', $ordenCompra->id) }}" class="btn-base bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 text-sm">Ver Detalle</a>
                    <a href="{{ route('ordenes_compra.historial') }}" class="btn-base bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 text-sm">Volver</a>
                </div>
            </div>
            ...
            <div class="border p-6 mb-6 rounded-lg shadow bg-gray-50">
                ...
                <form id="orden-form" action="{{ route('ordenes_compra.update', $ordenCompra->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="requisicion_id" value="{{ $ordenCompra->requisicion_id }}">

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-600 mb-2">Observaciones</label>
                        <textarea name="observaciones" rows="3" class="w-full border rounded-lg p-3 focus:ring-2 focus:ring-blue-400" placeholder="Observaciones adicionales...">{{ $ordenCompra->observaciones ?? '' }}</textarea>
                    </div>
                    ...
                    <div class="overflow-x-auto thin-scrollbar">
                        <table class="w-full border border-gray-200 rounded-lg overflow-hidden bg-white">
                            ...
                            <tbody id="productos-tbody">
                                @forelse($ordenCompra->ordencompraProductos as $linea)
                                @php
                                    $producto = $linea->producto;
                                    $lineaCentros = $ordenCompra->distribucionCentrosProductos->where('producto_id', $linea->producto_id);
                                @endphp
                                <tr data-producto-id="{{ $linea->producto_id }}" class="border-t">
                                    <td class="p-3">
                                        <div class="font-medium">{{ $producto->name_produc ?? 'Producto #' . $linea->producto_id }}</div>
                                        <input type="hidden" name="productos[{{ $linea->producto_id }}][id]" value="{{ $linea->producto_id }}">
                                    </td>
                                    <td class="p-3 text-center">
                                        <input type="text" name="productos[{{ $linea->producto_id }}][unidad]" value="{{ $producto->unit_produc ?? '-' }}" class="w-full border rounded p-2 text-center text-sm editable-unidad" placeholder="Unidad">
                                    </td>
                                    <td class="p-3 text-center">
                                        <input type="number" name="productos[{{ $linea->producto_id }}][cantidad]" value="{{ $linea->total }}" min="0" class="w-24 border rounded p-2 text-center" data-cantidad-original="{{ $linea->total }}">
                                    </td>
                                    <td class="p-3">
                                        <div class="space-y-2 max-h-40 overflow-y-auto thin-scrollbar">
                                            @forelse($lineaCentros as $dist)
                                                <div class="flex items-center justify-between bg-gray-50 px-2 py-1 rounded text-sm">
                                                    <span class="text-sm">{{ $dist->centro->name_centro ?? 'Centro #' . $dist->centro_id }}</span>
                                                    <input type="number" name="productos[{{ $linea->producto_id }}][centros][{{ $dist->centro_id }}]" value="{{ $dist->amount }}" min="0" class="w-16 border rounded p-1 text-center text-sm ml-2">
                                                </div>
                                            @empty
                                    ...
                                    <td class="p-3 text-center">
                                        <button type="button" class="btn-trash text-red-600 hover:bg-red-50" onclick="eliminarProducto({{ $linea->producto_id }})" title="Eliminar producto">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t">
                        <div class="text-sm text-gray-600">
                            <span id="total-productos">{{ $ordenCompra->ordencompraProductos->count() }}</span> producto(s) en la orden
                        </div>
                        <div class="flex gap-3">
                            <a href="{{ route('ordenes_compra.show', $ordenCompra->id) }}" class="btn-base bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2">Cancelar</a>
    ...
    <script>
        ...
        const productosData = {
            @foreach($productos as $p)
                {{ $p->id }}: { name: '{{ addslashes($p->name_produc) }}', unit: '{{ $p->unit_produc }}' },
            @endforeach
        };
